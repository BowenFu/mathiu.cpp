...
template <typename I, typename S> constexpr auto makeSubrange(I begin, S end) {
  return Subrange<I, S>{begin, end};
}

template <typename RangeType> class IterUnderlyingType {
public:
...
template <typename T, typename... Ts> class WithinTypes {
public:
  constexpr static auto value = (std::is_same_v<T, Ts> || ...);
};

template <typename T, typename Tuple> class PrependUnique;
...
template <typename Tuple> class Unique;

template <typename Tuple> using UniqueT = typename Unique<Tuple>::type;

template <> class Unique<std::tuple<>> {
public:
  using type = std::tuple<>;
};

template <typename T, typename... Ts> class Unique<std::tuple<T, Ts...>> {
public:
...
template <class T> using decayArrayT = typename decayArray<T>::type;
...
template <typename Pattern> class PatternTraits;

template <typename... PatternPairs> class PatternPairsRetType {
public:
...
template <typename Tuple> class Variant;

template <typename T, typename... Ts> class Variant<std::tuple<T, Ts...>> {
public:
  using type = std::variant<std::monostate, T, Ts...>;
};

template <typename... Ts> class Context {
...
public:
  template <typename T> constexpr void emplace_back(T &&t) {
...
template <typename T> class ContextTrait;

template <typename... Ts> class ContextTrait<std::tuple<Ts...>> {
public:
...
template <typename Pattern, typename Func> class PatternPair {
public:
...
template <typename Pattern, typename Pred> class PostCheck;

template <typename Pred> class When {
public:
  Pred mPred;
};

template <typename Pred> constexpr auto when(Pred const &pred) {
  return When<Pred>{pred};
}

template <typename Pattern> class PatternHelper {
public:
  constexpr explicit PatternHelper(Pattern const &pattern)
      : mPattern{pattern} {}
  template <typename Func> constexpr auto operator=(Func const &func) {
    return PatternPair<Pattern, Func>{mPattern, func};
  }
  template <typename Pred> constexpr auto operator|(When<Pred> const &w) {
...
template <typename... Patterns> class Ds;

template <typename... Patterns>
constexpr auto ds(Patterns const &...patterns) -> Ds<Patterns...>;

template <typename Pattern> class OooBinder;

class PatternPipable {
public:
  template <typename Pattern> constexpr auto operator|(Pattern const &p) const {
    return PatternHelper<Pattern>{p};
  }

  template <typename T> constexpr auto operator|(T const *p) const {
...
template <typename Pattern> class PatternTraits {
public:
  template <typename Value> using AppResultTuple = std::tuple<>;
...
template <> class PatternTraits<Wildcard> {
...
template <typename... Patterns> class Or {
public:
...
template <typename... Patterns> class PatternTraits<Or<Patterns...>> {
public:
  template <typename Value>
...
template <typename Pred> class Meet : public Pred {
public:
  using Pred::operator();
};

template <typename Pred> constexpr auto meet(Pred const &pred) {
  return Meet<Pred>{pred};
}

template <typename Pred> class PatternTraits<Meet<Pred>> {
public:
  template <typename Value> using AppResultTuple = std::tuple<>;
...
template <typename Unary, typename Pattern> class App {
public:
...
template <typename... Patterns> class And {
public:
...
template <typename Tuple> class NbIdInTuple;

template <typename... Patterns> class NbIdInTuple<std::tuple<Patterns...>> {
public:
  constexpr static auto nbIdV =
      (PatternTraits<std::decay_t<Patterns>>::nbIdV + ... + 0);
};

template <typename... Patterns> class PatternTraits<And<Patterns...>> {
public:
  template <typename Value>
  using AppResultTuple = decltype(std::tuple_cat(
...
template <typename Pattern> class Not {
public:
...
template <typename Pattern> constexpr auto not_(Pattern const &pattern) {
  return Not<Pattern>{pattern};
}

template <typename Pattern> class PatternTraits<Not<Pattern>> {
public:
  template <typename Value>
...
template <typename... Ts> class Overload : public Ts... {
public:
  using Ts::operator()...;
};

template <typename... Ts> constexpr auto overload(Ts &&...ts) {
...
template <typename Type> class Id {
private:
  class Block {
  public:
    ...
    constexpr auto hasValue() const {
      return std::visit(overload([](Type const &) { return true; },
                                 [](Type const *) { return true; },
...
public:
  ...
  template <typename Pattern> constexpr auto at(Pattern &&pattern) {
...
template <typename Type> class PatternTraits<Id<Type>> {
public:
  template <typename Value> using AppResultTuple = std::tuple<>;
...
private:
  template <typename T> struct AddConstToPointer {
...
template <typename T> class OooBinder {
  Id<T> mId;
...
class Ooo {
public:
  template <typename T> constexpr auto operator()(Id<T> id) const {
...
template <> class PatternTraits<Ooo> {
public:
  template <typename Value> using AppResultTuple = std::tuple<>;
...
template <typename Pattern> class PatternTraits<OooBinder<Pattern>> {
public:
  template <typename Value>
  using AppResultTuple =
      typename PatternTraits<Pattern>::template AppResultTuple<Value>;
...
template <typename T> class IsOoo : public std::false_type {};

template <> class IsOoo<Ooo> : public std::true_type {};

template <typename T> class IsOooBinder : public std::false_type {};
...
template <typename Tuple> constexpr size_t findOooIdx() {
...
template <std::size_t start, std::size_t end, class Tuple> class SubTypes {
...
template <typename ValueTuple> class IsArray : public std::false_type {};
...
template <typename... Patterns> class PatternTraits<Ds<Patterns...>> {
  constexpr static auto nbOooOrBinder = nbOooOrBinderV<Patterns...>;
  static_assert(nbOooOrBinder == 0 || nbOooOrBinder == 1);

public:
  template <typename PsTuple, typename VsTuple> class PairPV;
  ...
  template <typename Value, typename = std::void_t<>> class AppResultHelper;
